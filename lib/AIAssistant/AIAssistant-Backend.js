...
When user asks for something, generate creative, compact p5.js code that creates visual art matching their request.`;
  ...
  async initializeOllama() {
    try {
      ...
      document.getElementById('aiStatus').textContent = `AI: Ollama (${selectedModel}) Ready`;
      ...
      if (error.message.includes('Failed to fetch')) {
        errorMessage += 'Make sure Ollama is installed and running: 1) Install from https://ollama.ai 2) Run: ollama serve 3) Pull a model: ollama pull qwen3:0.6b';
      } else {
        errorMessage += error.message;
      }
