...
class QuillEditorManager {
  ...
  initializeQuill() {
    ...
    try {
      ...
      // Enhanced Quill configuration with spell check
      const quillConfig = {
      ...
      this.quillInstance = new Quill(editorElement, quillConfig);
      ...
      // Add spell check button to editor header
      this.addSpellCheckButton();
  ...
  // Method to enable spell check functionality
  enableSpellCheck() {
    console.log("🔤 Enabling spell check...");
    ...
    try {
      ...
      if (editorElement) {
        ...
        editorElement.setAttribute("autocapitalize", "sentences");

        console.log("✅ Native spell check enabled");

        // Add custom spell check styling
        this.addSpellCheckStyles();

        // Set up spell check event listeners
        this.setupSpellCheckEvents();
      }
    } catch (error) {
      console.error("❌ Failed to enable spell check:", error);
    }
  }

  // Add custom styles for spell check
  addSpellCheckStyles() {
    // Check if styles already exist
    if (document.getElementById("quill-spellcheck-styles")) {
      return;
    }

    const style = document.createElement("style");
    style.id = "quill-spellcheck-styles";
    style.textContent = `
      ...
      /* Browser spell check styling */
      .ql-editor *::-webkit-spelling-error {
        text-decoration: underline wavy #e53e3e;
        text-decoration-thickness: 2px;
      }
      
      .ql-editor *::-webkit-grammar-error {
        text-decoration: underline wavy #38a169;
        text-decoration-thickness: 2px;
      }
      
      .ql-editor *::-moz-spelling-error {
        text-decoration: underline wavy #e53e3e;
        text-decoration-thickness: 2px;
      }
      
      .ql-editor *::-moz-grammar-error {
        text-decoration: underline wavy #38a169;
        text-decoration-thickness: 2px;
      }
      
      /* Spell check context menu styling */
      .ql-editor::selection {
        background-color: rgba(79, 172, 254, 0.2);
      }
      
      /* Custom spell check tooltip */
      .spell-check-tooltip {
        position: absolute;
        background: #2d3748;
        color: white;
      ...
      .spell-check-tooltip.show {
        opacity: 1;
      }
      
      .spell-check-tooltip::after {
        content: '';
        position: absolute;
        top: 100%;
        left: 50%;
    ...
    console.log("✅ Spell check styles added");
  }

  // Set up spell check event listeners
  setupSpellCheckEvents() {
    const editorElement = this.quillInstance.root;

    // Listen for right-click to show spell check context menu
    editorElement.addEventListener("contextmenu", (e) => {
      this.handleSpellCheckContextMenu(e);
    });

    // Listen for text changes to update spell check
    this.quillInstance.on("text-change", () => {
      this.updateSpellCheck();
    });

    console.log("✅ Spell check event listeners set up");
  }

  // Handle spell check context menu
  handleSpellCheckContextMenu(event) {
    // Let the browser handle the native spell check context menu
    // This will show spelling suggestions automatically
    console.log("🔤 Spell check context menu triggered");
  }

  // Update spell check when text changes
  updateSpellCheck() {
    // Force browser to recheck spelling
    const editorElement = this.quillInstance.root;
    if (editorElement) {
      // Temporarily disable and re-enable spell check to force refresh
  ...
  // Method to toggle spell check on/off
  toggleSpellCheck() {
    const editorElement = this.quillInstance.root;
    if (editorElement) {
      const currentState = editorElement.getAttribute("spellcheck") === "true";
      const newState = !currentState;

      editorElement.setAttribute("spellcheck", newState.toString());
      editorElement.setAttribute("autocorrect", newState ? "on" : "off");
      editorElement.setAttribute(
        "autocapitalize",
        newState ? "sentences" : "off"
      );

      console.log(`🔤 Spell check ${newState ? "enabled" : "disabled"}`);

      // Show status message
      this.showSpellCheckStatus(
        newState ? "Spell Check Enabled" : "Spell Check Disabled",
        newState
      );
  ...
  // Method to show spell check status
  showSpellCheckStatus(message, isEnabled) {
    // Remove existing status if any
    const existingStatus = document.querySelector(".spell-check-status");
    if (existingStatus) {
    ...
    statusElement.className = "spell-check-status";
    statusElement.textContent = message;
    statusElement.style.color = isEnabled ? "#38a169" : "#e53e3e";

    // Add to editor container
    const editorContainer = this.quillInstance.root.parentElement;
    if (editorContainer) {
  ...
  // Method to get spell check status
  isSpellCheckEnabled() {
    const editorElement = this.quillInstance.root;
    return editorElement && editorElement.getAttribute("spellcheck") === "true";
  }

  // Method to force spell check refresh
  refreshSpellCheck() {
    if (this.isSpellCheckEnabled()) {
      this.updateSpellCheck();
      console.log("🔄 Spell check refreshed");
    }
  }
    ...
    patterns.forEach((pattern) => {
      ...
      if (matches) {
        ...
        detectedPatterns.push(
          `${pattern.name} (${matchCount} matches, +${patternScore} points)`
        );
        console.log(
...
`;
  ...
  // Add spell check toggle button to the editor header
  addSpellCheckButton() {
    const headerElement = this.container.querySelector(".quill-editor-header");
    if (headerElement) {
      // Check if button already exists
      if (headerElement.querySelector(".spell-check-toggle")) {
        return;
      }

      const spellCheckButton = document.createElement("button");
      spellCheckButton.className = "spell-check-toggle";
      spellCheckButton.innerHTML = `
        <span class="spell-check-icon">🔤</span>
        <span class="spell-check-text">Spell Check</span>
      `;

      spellCheckButton.addEventListener("click", () => {
        const isEnabled = this.toggleSpellCheck();
        spellCheckButton.classList.toggle("active", isEnabled);
        spellCheckButton.style.background = isEnabled
          ? "rgba(52, 152, 219, 0.2)"
          : "rgba(255, 255, 255, 0.1)";
      });

      // Add to header
      headerElement.appendChild(spellCheckButton);

      // Set initial state
      spellCheckButton.classList.add("active");
      spellCheckButton.style.background = "rgba(52, 152, 219, 0.2)";

      console.log("✅ Spell check button added to header");
    }
  }
}
