...
class VectorStore {
  ...
  async getEmbeddingFromText(text) {
    ...
    try {
      // Add timeout to prevent hanging - increased for larger documents
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('Embedding generation timeout after 15 seconds')), 15000);
      });
  ...
  async generateEmbeddings(documentId, content) {
    try {
      ...
      // Limit number of chunks to prevent freezing
      if (chunks.length > 50) {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        console.log(`📊 Processing chunk ${i + 1}/${chunks.length} for ${documentId}`);
  ...
  async readTextFile(file) {
    return new Promise((resolve, reject) => {
      ...
      if (file.size > 10 * 1024 * 1024) { // 10MB limit
        reject(new Error(`File too large: ${file.name} (${(file.size / 1024 / 1024).toFixed(1)}MB). Please use files under 10MB.`));
        return;
      }
      ...
      reader.onload = e => {
        try {
          ...
          if (content.length > 5 * 1024 * 1024) { // 5MB text limit
            reject(new Error(`Text content too large: ${content.length} characters. Please use smaller files.`));
            return;
          }
