  ...
  async parsePDF(file) {
    try {
      ...
      for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
        ...
        // Try to detect images on the page
        try {
          const operatorList = await page.getOperatorList();
          const imageOps = operatorList.fnArray.filter((fn, i) => 
          ...
          if (imageOps.length > 0) {
            ...
            fullText += `[Page ${pageNum} contains ${imageOps.length} image(s)] `;
          }
        } catch (imageError) {
          console.warn(`Could not detect images on page ${pageNum}:`, imageError);
        }
      }
      
      if (imageCount > 0) {
        fullText += `\n\n[PDF contains ${imageCount} total embedded image(s)]`;
      }
