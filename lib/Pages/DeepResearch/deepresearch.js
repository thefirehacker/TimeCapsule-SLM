...
`;
  ...
  async viewDocument(documentId) {
    try {
      ...
      // 🆕 Check if this is a generated research document
      const isGeneratedResearch = document.metadata?.type === 'generated_research';
      ...
      const title = isGeneratedResearch ? 
        `🔬 Generated Research: ${document.name}` : 
      ...
      const metadata = isGeneratedResearch ? `
        ...
        ">
          ...
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; margin-top: 10px;">
            <div><strong>Research Type:</strong> ${document.metadata.researchType}</div>
            <div><strong>Depth:</strong> ${document.metadata.researchDepth}</div>
            <div><strong>Topics:</strong> ${document.metadata.topicsCount}</div>
            <div><strong>AI Provider:</strong> ${document.metadata.aiProvider}</div>
            <div><strong>Generated:</strong> ${new Date(document.metadata.generatedAt).toLocaleDateString()}</div>
            <div><strong>Document Integration:</strong> ${document.metadata.hasDocumentIntegration ? 'Yes' : 'No'}</div>
          </div>
        </div>
      ` : `
        <div style="
