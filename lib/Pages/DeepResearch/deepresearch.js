...
class DeepResearchApp {
  constructor() {
    ...
    this.maxRetries = 20; // Maximum retries for initialization
  ...
  waitForDOMAndInitialize() {
    ...
    const checkElements = () => {
      ...
      if (missingElements.length === 0) {
        console.log(
          "✅ All critical DOM elements found, proceeding with initialization"
        );
        this.proceedWithInitialization();
      } else {
        this.initializationRetries++;
        console.log(
          `⏳ Missing elements: ${missingElements.join(", ")} (attempt ${
            this.initializationRetries
          }/${this.maxRetries})`
        );

        if (this.initializationRetries < this.maxRetries) {
          setTimeout(checkElements, 200); // Wait 200ms and try again
        } else {
          console.error(
            "❌ Max retries reached, proceeding anyway (some features may not work)"
          );
          this.proceedWithInitialization();
        }
      }
  ...
  proceedWithInitialization() {
    ...
    if (AIClass) {
      try {
        ...
        console.log(
          "✅ AIAssistant initialized successfully using:",
          AIClass.name
        );
      } catch (error) {
        console.error(
          "❌ Failed to initialize AIAssistant during startup:",
          error
        );
        this.aiAssistant = null;
      }
      ...
      console.warn(
        "⚠️ No AI Assistant class available during init, will retry on connect"
      );
      this.aiAssistant = null;
    }
    ...
    // Initialize Quill Editor with better error handling
    ...
    console.log("✅ DeepResearchApp initialization completed");
  }
  ...
  async initializeVectorStore() {
    try {
      ...
      if (window.VECTOR_STORE_DISABLED) {
        console.warn(
          "⚠️ Vector Store disabled due to Transformers.js loading failure"
        );
        this.updateStatus(
          "⚠️ Vector Store disabled - Transformers.js failed to load"
        );
        return;
      }
      ...
      if (window.VectorStore) {
        ...
        if (
          window.sharedVectorStore &&
          window.sharedVectorStore.isInitialized
        ) {
        ...
        this.updateStatus(
          "⚠️ Vector Store not available - document features disabled"
        );
        this.updateDocumentStatus();
      }
    } catch (error) {
      ...
      this.updateStatus(
        "❌ Vector Store initialization failed: " + error.message
      );
  ...
  initializeQuillEditor() {
    ...
    // First check if the target container exists
    const targetContainer = document.getElementById("outputContent");
    if (!targetContainer) {
      console.error("❌ Target container 'outputContent' not found in DOM");
      this.updateStatus("❌ Editor container not found");
      return;
    }

    // Wait for QuillEditorManager to be available
    if (typeof QuillEditorManager === "undefined") {
      console.warn("⚠️ QuillEditorManager not yet available, retrying...");
    ...
    if (typeof Quill === "undefined") {
      console.warn("⚠️ Quill.js not yet loaded, retrying...");
      setTimeout(() => this.initializeQuillEditor(), 200);
      return;
    }

    try {
      console.log("🔧 Creating QuillEditorManager instance...");

      this.quillEditor = new QuillEditorManager("outputContent", {
        theme: "snow",
        placeholder:
          "Start typing your research content or generate AI content...",
        readOnly: false, // Always editable
      });

      console.log("✅ Quill Editor initialized successfully");
      this.updateStatus("🖋️ Editor ready for research content");

      // Test the editor is working
      setTimeout(() => {
        this.testEditorFunctionality();
      }, 500);
    } catch (error) {
      console.error("❌ Failed to initialize Quill Editor:", error);
      this.updateStatus("❌ Editor initialization failed: " + error.message);
  ...
  testEditorFunctionality() {
    if (this.quillEditor && this.quillEditor.quillInstance) {
      console.log("🧪 Testing editor functionality...");

      // Test if editor is responsive
      try {
        this.quillEditor.focus();
        console.log("✅ Editor focus test passed");

        // Test content setting
        this.quillEditor.setContent(
          "🖋️ Editor is ready! You can start typing..."
        );
        setTimeout(() => {
          this.quillEditor.setContent(""); // Clear test content
          console.log("✅ Editor content test passed");
        }, 1000);
      } catch (testError) {
        console.error("❌ Editor functionality test failed:", testError);
      }
    }
  }
    ...
    if (container) {
      container.innerHTML = `
        ...
        ">
          ...
          <p>The rich text editor failed to initialize. Please refresh the page to try again.</p>
  ...
  setupEventListeners() {
    ...
    // Set up core event listeners first (don't wait for TimeCapsule buttons)
    this.setupCoreEventListeners();

    // Set up TimeCapsule buttons separately (these might not exist immediately)
    this.setupTimeCapsuleListeners();
  }
  ...
  setupCoreEventListeners() {
    ...
    if (connectBtn) {
      ...
      console.warn("⚠️ Connect AI button not found, will retry later");
    }
    ...
    if (topicTitleInput) {
      topicTitleInput.addEventListener("keypress", (e) => {
    ...
    if (generateBtn) {
      generateBtn.addEventListener("click", () => this.generateResearch());
    }

    const exportBtn = document.getElementById("exportResearch");
    if (exportBtn) {
      exportBtn.addEventListener("click", () => this.exportResearch());
    }

    const clearOutputBtn = document.getElementById("clearResearchOutput");
    if (clearOutputBtn) {
      clearOutputBtn.addEventListener("click", () =>
        this.clearResearchOutput()
      );
    }
    ...
    if (providerSelect) {
      providerSelect.addEventListener("change", () => this.updateAIProvider());
    }

    // Document Management
    const uploadDocsBtn = document.getElementById("uploadDocuments");
    if (uploadDocsBtn) {
      uploadDocsBtn.addEventListener("click", () => this.showDocumentUpload());
    }

    const uploadRepoBtn = document.getElementById("uploadRepository");
    if (uploadRepoBtn) {
      uploadRepoBtn.addEventListener("click", () => this.showRepositoryInput());
    }

    const manageDocsBtn = document.getElementById("manageDocuments");
    if (manageDocsBtn) {
      manageDocsBtn.addEventListener("click", () => this.showDocumentManager());
    }
  ...
  setupTimeCapsuleListeners() {
    // Handle TimeCapsule buttons separately with retries
    const setupTimeCapsuleButtons = () => {
      ...
      if (exportBtn && loadBtn && loadInput) {
        ...
        console.log("✅ TimeCapsule event listeners attached");
        this.eventListenersSetup = true;
      } else {
        // These buttons might not exist, which is okay
        console.log("ℹ️ TimeCapsule buttons not found (this is optional)");
        this.eventListenersSetup = true;
      }
    };

    setupTimeCapsuleButtons();
  }
  ...
  setupResizeHandle() {
    ...
    document.addEventListener("mousemove", (e) => {
      ...
      const editorRatio = Math.max(
        0.1,
        Math.min(0.9, mouseInAvailable / availableWidth)
      );
    ...
    document.addEventListener("mouseup", () => {
      if (isResizing) {
        ...
        this.updateStatus(
          `Layout: ${currentEditorFr.toFixed(1)}:${currentCanvasFr.toFixed(
            1
          )} ratio`
        );
      }
  ...
  async connectAI() {
    ...
    try {
      ...
      if (!this.aiAssistant) {
        console.log(
          "🔄 AIAssistant not initialized during startup, attempting initialization..."
        );
        ...
        if (AIClass) {
          try {
            ...
            this.aiAssistant.onStatusChange = (status) =>
              this.updateAIStatus(status);
            console.log(
              "✅ AIAssistant initialized on connect using:",
              AIClass.name
            );
          } catch (initError) {
            console.error(
              "❌ Failed to initialize AIAssistant on connect:",
              initError
            );
            throw new Error(
              "Failed to initialize AI Assistant: " + initError.message
            );
          }
        } else {
          // Final check - see what's actually available
          console.error(
            "🔍 Final debug - window.AIAssistant:",
            !!window.AIAssistant
          );
          console.error(
            "🔍 window.AIAssistantBackend:",
            !!window.AIAssistantBackend
          );
          console.error(
            "🔍 All AI-related globals:",
            Object.keys(window).filter((key) =>
              key.toLowerCase().includes("ai")
            )
          );
          throw new Error(
            "AI Assistant class not available. Please refresh the page and try again."
          );
        }
      }
      ...
      if (
        provider === "ollama" &&
        this.isOllamaConnectionError(error.message)
      ) {
  ...
  updateAIStatus(status) {
    ...
    if (status.connected) {
      ...
      if (headerStatus)
        headerStatus.textContent = `AI Connected (${status.provider})`;
      this.updateStatus(
        `✅ AI Connected (${status.provider}) - Ready to generate research!`
      );
  ...
  renderTopics() {
    ...
    structureList.innerHTML = this.topics
      .map(
        (topic, index) => `
      <div class="structure-item" data-topic-id="${topic.id}">
        <div class="structure-item-content" onclick="deepResearch.selectTopic(${
          topic.id
        })">
        ...
        <div class="structure-item-actions">
          <button class="action-btn" onclick="deepResearch.moveTopic(${
            topic.id
          }, 'up')" title="Move Up">↑</button>
          <button class="action-btn" onclick="deepResearch.moveTopic(${
            topic.id
          }, 'down')" title="Move Down">↓</button>
          <button class="action-btn delete" onclick="deepResearch.deleteTopic(${
            topic.id
  ...
  moveTopic(topicId, direction) {
    ...
    // Swap topics
    [this.topics[topicIndex], this.topics[newIndex]] = [
      this.topics[newIndex],
  ...
  updateGenerateButton() {
    ...
    const hasDocuments =
      this.vectorStore &&
      this.vectorStore.isInitialized &&
    ...
    if (this.isGenerating) {
      if (hasDocuments) {
        generateBtn.innerHTML =
          '<span class="loading-spinner"></span> Generating with Documents...';
      } else {
        generateBtn.innerHTML =
      ...
      generateBtn.innerHTML = `🔌 Connect ${
        providerNames[provider] || "AI"
      } First`;
    } else if (!hasTopics) {
  ...
  updateDocumentStatus() {
    ...
    if (hasDocuments) {
      ...
      statusDetails.textContent =
        "Transformers.js failed to load - refresh page to retry";
  ...
  async generateResearch() {
    ...
    if (!this.aiAssistant) {
      this.updateStatus(
        "❌ AI Assistant not initialized - please refresh the page and try again"
      );
      return;
    }
      ...
      try {
        ...
        if (storedConnection) {
          ...
          if (connectionData.connected && connectionAge < 30 * 60 * 1000) {
            this.updateStatus(
              "🔄 AI connection detected but not active - attempting to reconnect..."
            );
            ...
            try {
              ...
              this.updateStatus(
                "❌ Failed to reconnect AI - please connect manually"
              );
              localStorage.removeItem("deepResearch_aiConnected");
              this.updateGenerateButton();
              return;
    ...
    try {
      ...
      if (hasDocuments) {
        ...
        this.updateStatus(
          `🔍 Searching ${docCount} documents (${vectorCount} chunks) for relevant context...`
        );
      } else {
      ...
      if (hasDocuments) {
        this.updateStatus(
          "🤖 Generating enhanced research report with document insights..."
        );
      } else {
      ...
      const response = await this.aiAssistant.generateContent(
        prompt,
        "research"
      );
      ...
      if (content) {
        ...
        this.researchResults.documentCount = hasDocuments
          ? this.vectorStore.collections.documents.size
          : 0;
        ...
        if (hasDocuments) {
          const docCount = this.vectorStore.collections.documents.size;
          this.updateStatus(
            `✅ Enhanced research report generated successfully with insights from ${docCount} documents!`
          );
        } else {
  ...
  async buildResearchPrompt(researchType, researchDepth) {
    const topicsList = this.topics
      .map(
        (topic, index) => `${index + 1}. ${topic.title} - ${topic.description}`
      )
      .join("\n");
    ...
    const depthInstructions = {
      ...
      detailed:
        "Provide detailed analysis with comprehensive explanations and examples.",
      comprehensive:
        "Provide comprehensive research with deep analysis, multiple perspectives, and extensive coverage.",
    };

    const typeInstructions = {
      academic:
        "Focus on scholarly sources, theoretical frameworks, and rigorous analysis.",
      market:
        "Focus on market trends, competitive landscape, and business implications.",
      technology:
        "Focus on technical specifications, innovations, and future developments.",
      competitive:
        "Focus on competitor analysis, market positioning, and strategic insights.",
      trend:
        "Focus on emerging trends, future predictions, and impact analysis.",
      literature:
        "Focus on existing literature, research findings, and knowledge synthesis.",
    };
    ...
    if (this.vectorStore && this.vectorStore.isInitialized) {
      try {
        ...
        const allTopicsText = this.topics
          .map((t) => `${t.title} ${t.description}`)
          .join(" ");
        allSearches.push(
          this.vectorStore
            .search(allTopicsText, {
              limit: 8,
        ...
        const typeKeywords = {
          academic:
            "research study analysis method methodology findings conclusion",
          market:
            "market trends competition business industry analysis revenue growth",
          technology:
            "technology innovation development technical specifications features",
          competitive:
            "competitor competition analysis market share strategy positioning",
          trend:
            "trend emerging future prediction forecast development pattern",
          literature:
            "literature review research paper study findings analysis summary",
        };

        if (typeKeywords[researchType]) {
          allSearches.push(
            this.vectorStore
              .search(typeKeywords[researchType], {
                limit: 4,
                minSimilarity: 0.2,
              })
        ...
        const timeoutPromise = new Promise((_, reject) => {
          setTimeout(
            () =>
              reject(
                new Error(
                  "Context search timeout - document search took too long"
                )
              ),
            45000
          );
        ...
        console.log(
          `🔍 Starting enhanced search across ${allSearches.length} search queries...`
        );
        ...
        for (const searchResult of searchResults) {
          if (
            ...
            for (const doc of results) {
              ...
              if (
                !allRelevantDocs.has(key) ||
                allRelevantDocs.get(key).similarity < doc.similarity
              ) {
        ...
        if (sortedDocs.length > 0) {
          const highRelevance = sortedDocs.filter(
            (doc) => doc.similarity >= 0.4
          );
          const mediumRelevance = sortedDocs.filter(
            (doc) => doc.similarity >= 0.25 && doc.similarity < 0.4
          );
          const lowRelevance = sortedDocs.filter(
...
${
  highRelevance.length > 0
    ? `🔴 HIGH RELEVANCE SOURCES (${highRelevance.length}):
${highRelevance
  .map(
    (doc, index) =>
      `${index + 1}. 📄 "${doc.document?.name || "Unknown Document"}" (Chunk ${
        doc.chunkIndex + 1
...
`
    : ""
            ...
            mediumRelevance.length > 0
              ? `🟡 MEDIUM RELEVANCE SOURCES (${mediumRelevance.length}):
${mediumRelevance
  .map(
    (doc, index) =>
      `${index + 1}. 📄 "${doc.document?.name || "Unknown Document"}" (${(
        doc.similarity * 100
      ).toFixed(1)}% match)
...
`
            ...
            lowRelevance.length > 0
              ? `🟢 SUPPORTING SOURCES (${lowRelevance.length}):
${lowRelevance
  .slice(0, 5)
  .map(
    (doc, index) =>
...
`
          ...
          console.log(
            `✅ Enhanced context search found ${sortedDocs.length} relevant excerpts from ${totalResults} total matches`
          );
          console.log(
            `📊 Relevance breakdown: ${highRelevance.length} high, ${mediumRelevance.length} medium, ${lowRelevance.length} supporting`
          );
        } else {
        ...
        if (error.message.includes("timeout")) {
          console.warn(
            "⚠️ Context search timed out after 45 seconds - continuing without document context"
          );
          contextSection = `
...
Generate a thorough, well-structured research report that seamlessly integrates your AI knowledge with the specific insights from the uploaded documents.
  ...
  generateSources(hasDocuments = false) {
    const docInfo =
      hasDocuments && this.vectorStore ? this.vectorStore.getStats() : null;
...
${
  hasDocuments
    ? `2. **Uploaded Document Knowledge Base**: ${docInfo.documents} uploaded documents with ${docInfo.vectors} searchable text chunks
3. **Semantic Search Integration**: Vector-based similarity matching to find relevant content`
    : "2. **Public Knowledge**: Established information and best practices"
}
...
${
  this.vectorStore
    ? this.getDocumentSourcesSummary()
...
${
  hasDocuments
    ? `- **Uploaded Documents**: User-provided materials with direct content extraction and analysis
...
${
  hasDocuments
    ? `**High Reliability**: Direct quotes and data from uploaded documents provide primary source validation.
**Medium Reliability**: AI synthesis of uploaded content with cross-document validation.
**Standard Reliability**: General AI knowledge cross-referenced where possible.`
    : `All information has been cross-referenced and validated against multiple sources where possible.`
}
...
${
  hasDocuments
    ? `Documents integrated: ${docInfo.documents}
...
${
  hasDocuments
    ? `*Note: This research integrates both AI knowledge and your specific uploaded documents for enhanced accuracy and relevance.*`
...
${
  hasDocuments
    ? `- **Document Integration**: ${docInfo.documents} documents, ${docInfo.vectors} searchable chunks
- **Vector Store**: Semantic search enabled with similarity matching`
    : "- **Document Integration**: No documents uploaded"
}
...
${
  hasDocuments
    ? "- [ ] Upload additional relevant documents if needed"
    : "- [ ] Consider uploading relevant documents for enhanced analysis"
}
- [ ] Share results with stakeholders
- [ ] Plan follow-up research phases
${
  hasDocuments
    ? "- [ ] Update document knowledge base as new information becomes available"
    : ""
}
...
- **Source Diversity**: Consider adding documents from different perspectives or time periods`
    : `## 💡 Research Enhancement Tips
...
- **Current Information**: Add recent reports or articles for up-to-date insights`
  ...
  getDocumentSourcesSummary() {
    if (!this.vectorStore || !this.vectorStore.collections)
  ...
  getDocumentQualityNotes() {
    ...
    if (vectors.length > 0) {
      avgChunkSize = Math.round(
        vectors.reduce((sum, v) => sum + (v.content?.length || 0), 0) /
          vectors.length
      );
    }
...
- **Content Distribution**: Evenly distributed across ${
      docs.length
    } source documents
- **Recommendation**: ${
      totalChunks > 50 ? "Excellent" : totalChunks > 20 ? "Good" : "Limited"
    } document coverage for comprehensive analysis`;
  }

  renderOutput() {
    if (!this.quillEditor) {
      console.warn("⚠️ Quill editor not yet initialized, retrying...");
      setTimeout(() => this.renderOutput(), 100);
      return;
    }

    console.log("🔄 renderOutput called");
    console.log("📋 Current tab:", this.currentTab);
    console.log(
      "📊 Research results available:",
      Object.keys(this.researchResults)
    );

    const content = this.researchResults[this.currentTab];

    console.log(
      `📝 Content for ${this.currentTab}:`,
      content ? content.substring(0, 200) + "..." : "None"
    );

    if (content && content.trim()) {
      console.log(`🖋️ Loading content into ${this.currentTab} tab`);
      console.log(`📝 Content length: ${content.length} characters`);

      // Set content with explicit debugging
      this.quillEditor.setContent(content, this.currentTab);

      // Verify content was set
      setTimeout(() => {
        const editorContent = this.quillEditor.getContent(this.currentTab);
        console.log(
          `✅ Content verification for ${this.currentTab}:`,
          editorContent ? editorContent.substring(0, 100) + "..." : "Empty"
        );
      }, 100);

      this.updateStatus(`✅ ${this.currentTab} content loaded into editor`);
    } else {
      console.log(
        `📝 No content for ${this.currentTab} tab, showing empty state`
      );
      this.quillEditor.setContent("", this.currentTab);
    }
  }

  renderEmptyOutput() {
    if (!this.quillEditor) {
      console.warn("⚠️ Quill editor not yet initialized for empty state");
      return;
    }

    console.log("📝 Rendering empty state in editor");
    this.quillEditor.setContent("", this.currentTab);
  }
...
`;
  ...
  clearResearchOutput() {
    ...
    if (
      confirm(
        "Are you sure you want to clear the research output? Your topics will be preserved."
      )
    ) {
      this.researchResults = {};
  ...
  clearAll() {
    if (
      confirm("Are you sure you want to clear all topics and research results?")
    ) {
      this.topics = [];
      this.researchResults = {};
      this.renderTopics();
      ...
      if (data) {
        ...
        if (parsed.settings) {
          document.getElementById("researchType").value =
            parsed.settings.researchType || "academic";
          document.getElementById("researchDepth").value =
            parsed.settings.researchDepth || "detailed";
          document.getElementById("aiProviderSelect").value =
            parsed.settings.aiProvider || "ollama";
        }
  ...
  showDocumentUpload() {
    if (!this.vectorStore) {
      if (window.VECTOR_STORE_DISABLED) {
        this.updateStatus(
          "❌ Document features disabled - Transformers.js failed to load"
        );
      } else {
  ...
  showRepositoryInput() {
    this.updateStatus(
      '🚧 GitHub repository integration coming soon! Please use "📄 Upload Documents" for now.'
    );
  }

  async handleRepositoryUpload(repoUrl) {
    this.updateStatus(
      "🚧 GitHub repository integration coming soon! Please upload individual files for now."
    );
  }
  ...
  showDocumentManager() {
    if (!this.vectorStore) {
      if (window.VECTOR_STORE_DISABLED) {
        alert(
          "❌ Document features are disabled because Transformers.js failed to load.\n\nPlease:\n1. Check your internet connection\n2. Refresh the page\n3. Wait for the embedding model to download (~23MB)"
        );
      } else {
        this.updateStatus("❌ Vector Store not initialized");
      }
  ...
  async handleModalDocumentUpload(event) {
    ...
    try {
      ...
      try {
        ...
        console.log(
          "ℹ️ Could not clear file input (not critical):",
          clearError.message
        );
      }
    } catch (error) {
    ...
    try {
      ...
      documentList.innerHTML = documents
        .map((doc) => {
          ...
          const chunkCount = Array.from(
            this.vectorStore.collections.vectors.values()
          ...
          <div class="document-item">
            <div class="document-info">
              ...
              <div class="document-meta">
                Type: ${doc.type} • Size: ${this.formatFileSize(
            doc.size
          )} • Added: ${new Date(doc.createdAt).toLocaleDateString()}
                ${
                  chunkCount > 0
                    ? `<br>📑 Processed into ${chunkCount} searchable chunk${
                        chunkCount > 1 ? "s" : ""
                      }`
                    : ""
                }
            ...
            <div class="document-actions">
              <button class="doc-action-btn" onclick="deepResearch.previewDocument('${
                doc.id
              }')">👁️ View</button>
              <button class="doc-action-btn delete" onclick="deepResearch.deleteDocument('${
                doc.id
              }')">🗑️ Delete</button>
            </div>
          </div>
  ...
  updateVectorStats() {
    ...
    try {
      if (
        ...
        document.getElementById("statRepositories").textContent =
          stats.repositories;
        return;
      }
      ...
      modalContent.innerHTML = `
        ...
        <div style="background: rgba(0,0,0,0.3); border-radius: 10px; padding: 20px; max-height: 500px; overflow-y: auto;">
          <div style="color: rgba(255,255,255,0.7); margin-bottom: 15px; font-size: 14px;">
            📊 <strong>Document Info:</strong> ${this.escapeHtml(
              doc.type
            )} • ${this.formatFileSize(doc.size)} • ${new Date(
        doc.createdAt
      ).toLocaleDateString()}
          </div>
          <pre style="white-space: pre-wrap; color: white; font-family: monospace;">${this.escapeHtml(
            doc.content
          )}</pre>
        </div>
      `;
  ...
  previewChunk(documentId, chunkIndex, chunkContent) {
    try {
      ...
      modalContent.innerHTML = `
        ...
        chunkIndex + 1
      } • <strong>Length:</strong> ${chunkContent.length} characters
          </div>
          ...
          <pre style="white-space: pre-wrap; color: white; font-family: 'Segoe UI', system-ui, sans-serif; line-height: 1.5;">${this.escapeHtml(
            chunkContent
          )}</pre>
        </div>
  ...
  async searchDocuments() {
    ...
    const thresholdInput = prompt(
      "Similarity threshold (0.1-0.9, lower = more results):",
      "0.2"
    );
    ...
    try {
      ...
      try {
        ...
        modalContent.innerHTML = `
          <div class="document-modal-header">
            ...
            <div style="font-size: 12px; color: rgba(255,255,255,0.7); margin-top: 5px;">
              Found ${results.length} results • Threshold: ${(
          threshold * 100
        ).toFixed(1)}%
            </div>
            <button class="close-modal">✕</button>
        ...
        results.forEach((result, index) => {
          console.log(
            `${index + 1}. ${result.document?.name || "Unknown"} (${(
        ...
        this.updateStatus(
          "✅ Search completed - results shown in fallback mode"
        );
      }
    } catch (error) {
          ...
          resultItem.innerHTML = `
            <div class="document-info">
              ...
              <div class="document-meta" style="margin-top: 10px; padding: 10px; background: rgba(0,0,0,0.2); border-radius: 5px;">
                ...
                "${this.escapeHtml(content.substring(0, 250))}${
            content.length > 250 ? "..." : ""
          }"
              </div>
            </div>
          ...
          if (chunkBtn) {
            chunkBtn.addEventListener("click", (e) => {
              ...
              try {
                ...
                this.updateStatus(
                  "❌ Error previewing chunk: " + error.message
                );
              }
            });
          }
          ...
          if (docBtn) {
            docBtn.addEventListener("click", (e) => {
              ...
              try {
                ...
                this.updateStatus(
                  "❌ Error previewing document: " + error.message
                );
              }
            });
          }
  ...
  async clearVectorStore() {
    if (
      !confirm(
        "Are you sure you want to clear all documents and vectors? This action cannot be undone."
      )
    ) {
      return;
    }
  ...
  async quickSearch() {
    ...
    try {
      ...
      try {
        ...
        this.showSimpleSearchResults(results, query, threshold);
        this.updateStatus(
          "⚡ Quick search completed - results shown in fallback mode"
        );
      }
    } catch (error) {
  ...
  showSimpleSearchResults(results, query, threshold = 0.3) {
    ...
    try {
      ...
      alert(
        `Search found ${results.length} results but display failed. Check console for details.`
      );
    }
  }
  ...
  loadTimeCapsule(event) {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (data.metadata) {
          if (data.metadata.researchType)
            document.getElementById("researchType").value =
              data.metadata.researchType;
          if (data.metadata.researchDepth)
            document.getElementById("researchDepth").value =
              data.metadata.researchDepth;
          if (data.metadata.aiProvider)
            document.getElementById("aiProviderSelect").value =
              data.metadata.aiProvider;
        }
        this.renderTopics();
        this.renderOutput();
        this.updateGenerateButton();
...
window.collapseStructure = function () {
  document
    .querySelectorAll(".structure-item:not(.selected)")
    .forEach((item) => {
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", () => {
    if (
      window.deepResearch &&
      typeof window.deepResearch.setupEventListeners === "function"
    ) {
