...
class DeepResearchApp {
  constructor() {
    ...
    // Don't call init() here since it's async - will be called from HTML
  }
  ...
  async init() {
    ...
    // Check for existing shared AI connection first
    ...
    if (window.SharedAIConnection) {
      ...
      if (window.SharedAIConnection.isAIConnected()) {
        ...
        if (this.aiAssistant) {
          ...
          this.updateStatus(`✅ Using existing AI connection (${currentProvider.provider})`);
        }
      } else {
    ...
    // Ensure button text is correct after initialization
    setTimeout(() => {
      this.updateConnectButtonText();
    }, 100);
  }
  ...
  setupEventListeners() {
    ...
    if (connectBtn) {
      connectBtn.addEventListener('click', () => {
        ...
        if (window.AppConfig && typeof window.AppConfig.trackKeyEvent === 'function') {
          ...
          window.AppConfig.trackKeyEvent('connect_ai_clicked', 'deepresearch', {
            ai_provider: provider,
            page_context: 'deepresearch'
          });
        }
    ...
    document.getElementById('addTopic').addEventListener('click', () => {
      if (window.AppConfig && typeof window.AppConfig.trackEvent === 'function') {
        window.AppConfig.trackEvent('add_topic_clicked', 'content_creation', 'deepresearch');
      }
      this.addTopic();
    });
    document.getElementById('topicTitle').addEventListener('keypress', (e) => {
      if (e.key === 'Enter') {
        if (window.AppConfig && typeof window.AppConfig.trackEvent === 'function') {
          window.AppConfig.trackEvent('add_topic_enter', 'content_creation', 'deepresearch');
        }
        this.addTopic();
      }
    });
    ...
    // Research Actions
    document.getElementById('generateResearch').addEventListener('click', () => {
      if (window.AppConfig && typeof window.AppConfig.trackKeyEvent === 'function') {
        const researchType = document.getElementById('researchType')?.value || 'unknown';
        const researchDepth = document.getElementById('researchDepth')?.value || 'unknown';
        window.AppConfig.trackKeyEvent('generate_research_clicked', 'deepresearch', {
    ...
    document.getElementById('uploadDocuments').addEventListener('click', () => {
      if (window.AppConfig && typeof window.AppConfig.trackKeyEvent === 'function') {
        window.AppConfig.trackKeyEvent('upload_documents_clicked', 'deepresearch', {
          page_context: 'deepresearch'
        });
      }
    ...
    document.getElementById('manageDocuments').addEventListener('click', () => {
      if (window.AppConfig && typeof window.AppConfig.trackKeyEvent === 'function') {
        window.AppConfig.trackKeyEvent('manage_knowledge_clicked', 'deepresearch', {
          page_context: 'deepresearch'
        });
      }
      this.showDocumentManager();
    });
  ...
  setupSharedConnectionListener() {
    if (window.SharedAIConnection) {
      ...
      window.SharedAIConnection.addConnectionListener((event, data) => {
        console.log('🔄 DeepResearch received shared connection event:', event, data);
        ...
        if (event === 'connected') {
          ...
          this.updateConnectButtonText();
          this.updateStatus(`✅ AI connected via ${providerInfo.provider}`);
          ...
          this.updateConnectButtonText();
          this.updateStatus('❌ AI connection error: ' + data.message);
        }
      });
    }
  }
    ...
    try {
      ...
      if (window.SharedAIConnection) {
        ...
        const connectionListener = (event, data) => {
          ...
          if (event === 'connected') {
            ...
            this.updateAIStatus({
              connected: true,
              provider: window.SharedAIConnection.currentProvider
            });
            
            connectBtn.disabled = false;
            this.updateConnectButtonText();
            this.updateStatus(`✅ AI Connected (${window.SharedAIConnection.currentProvider})`);
          } else if (event === 'error') {
            this.updateAIStatus({
              connected: false,
              error: data.message
            ...
            connectBtn.disabled = false;
            this.updateConnectButtonText();
            this.updateStatus(`❌ AI Connection Failed: ${data.message}`);
          } else if (event === 'disconnected') {
            this.aiAssistant = null;
            this.updateAIStatus({
              connected: false
        ...
        connectBtn.disabled = false;
        this.updateConnectButtonText();
        this.updateStatus('🔌 Please complete connection in the modal');
      ...
      if (!this.aiAssistant) {
        ...
        if (AIClass) {
          this.aiAssistant = new AIClass();
          this.aiAssistant.onStatusChange = (status) => this.updateAIStatus(status);
          console.log('✅ AIAssistant initialized locally using:', AIClass.name);
        } else {
          throw new Error('AI Assistant class not available. Please refresh the page and try again.');
        }
      }
      
      // Set the provider first, then show the modal
      ...
      this.updateStatus('❌ AI connection setup failed: ' + error.message);
  ...
  updateConnectButtonText() {
    ...
    // Fallback to local AI assistant check
    else if (this.aiAssistant && this.aiAssistant.isConnected) {
      isConnected = true;
    }
    // Final fallback to stored connection state
    else {
      try {
        ...
        if (storedConnection) {
          ...
          isConnected = connectionData.connected && connectionAge < (30 * 60 * 1000);
        }
      } catch (error) {
    ...
    if (isConnected) {
      // Get current provider for connected state
      ...
      connectBtn.innerHTML = connectedLabels[currentProvider] || '✅ AI Connected';
    } else {
  ...
  updateAIStatus(status) {
    ...
    if (status.connected) {
      ...
      statusEl.innerHTML = `<div>AI Connection Issue</div><div style="font-size: 12px; color: rgba(255,255,255,0.8); margin-top: 5px;">${status.error}</div>`;
      if (headerStatus) headerStatus.textContent = 'AI Connection Issue';
      this.updateStatus(`AI Connection Issue: ${status.error}`);
    } else {
    ...
    // Always update connect button text when AI status changes
    this.updateConnectButtonText();
  ...
  async generateResearch() {
    ...
    try {
      ...
      if (content) {
        ...
        // 🆕 AUTOMATICALLY SAVE RESEARCH OUTPUT TO RAG VECTOR STORE
        await this.saveResearchToVectorStore(content);
  ...
  /**
   * Save research output to vector store for future reference
   * This creates a cumulative knowledge base of previous research
   */
  async saveResearchToVectorStore(researchContent) {
    // Only save if vector store is available and initialized
    if (!this.vectorStore || !this.vectorStore.isInitialized) {
      console.log('ℹ️ Vector store not available - skipping research output storage');
      return;
    }
    
    try {
      console.log('💾 Saving research output to vector store for future reference...');
      ...
      // Build comprehensive content including metadata
      const topicsList = this.topics.map((topic, index) => 
        `${index + 1}. ${topic.title} - ${topic.description}`
      ).join('\n');
      
      const fullResearchDocument = `# Research Report: ${researchType.charAt(0).toUpperCase() + researchType.slice(1)} Analysis
...
- **Document Integration**: ${this.researchResults.hasDocuments ? `Yes (${this.researchResults.documentCount} documents)` : 'No'}
...
---
*This research report was automatically generated and stored for future reference and cross-analysis.*`;

      // Create a virtual file object for the research document
      const researchFileName = `research-${researchType}-${Date.now()}.md`;
      const researchBlob = new Blob([fullResearchDocument], { type: 'text/markdown' });
      const researchFile = new File([researchBlob], researchFileName, { type: 'text/markdown' });
      
      // Add metadata to identify this as a generated research document
      const options = {
        metadata: {
          type: 'generated_research',
          researchType: researchType,
          researchDepth: researchDepth,
          topicsCount: this.topics.length,
          topics: this.topics.map(t => ({ title: t.title, description: t.description })),
          generatedAt: timestamp,
          hasDocumentIntegration: this.researchResults.hasDocuments,
          documentCount: this.researchResults.documentCount || 0,
      ...
      const docId = await this.vectorStore.addDocument(researchFile, options);
      
      console.log(`✅ Research output saved to vector store with ID: ${docId}`);
      console.log(`📊 Future research can now reference this ${researchType} analysis`);
      ...
      // Show user feedback
      const stats = this.vectorStore.getStats();
      this.updateStatus(`✅ Research saved to knowledge base! (${stats.documents} total documents now available for future research)`);
      
    } catch (error) {
      console.error('❌ Failed to save research output to vector store:', error);
      console.warn('⚠️ Research generation completed successfully, but saving to vector store failed');
      // Don't throw error - research generation was successful, storage is just a bonus feature
    }
  }
  ...
  async buildResearchPrompt(researchType, researchDepth) {
    ...
    if (this.vectorStore && this.vectorStore.isInitialized) {
      try {
        ...
        // 🆕 Search 4: Previous research outputs (NEW FEATURE)
        allSearches.push(
          this.vectorStore.search(`research report ${researchType} analysis findings`, {
            limit: 3,
            minSimilarity: 0.25
          }).then(results => ({ type: 'previous_research', results }))
        );
        
        // 🆕 Search 5: Cross-research topic analysis (NEW FEATURE)
        const topicKeywords = this.topics.map(t => t.title).join(' ');
        allSearches.push(
          this.vectorStore.search(`${topicKeywords} research analysis study`, {
            limit: 3,
            minSimilarity: 0.3
          }).then(results => ({ type: 'related_topics', results }))
        );
        ...
        if (sortedDocs.length > 0) {
          ...
          // 🆕 Separate previous research from regular documents
          const previousResearch = sortedDocs.filter(doc => 
            doc.searchType === 'previous_research' || 
            doc.document?.metadata?.type === 'generated_research'
          );
          const regularDocs = sortedDocs.filter(doc => 
            doc.searchType !== 'previous_research' && 
            doc.document?.metadata?.type !== 'generated_research'
          );
...
${previousResearch.length > 0 ? `🔬 PREVIOUS RESEARCH FINDINGS (${previousResearch.length}):
${previousResearch.map((doc, index) => {
  const metadata = doc.document?.metadata;
  const researchInfo = metadata?.type === 'generated_research' 
    ? `\n   📋 Research Type: ${metadata.researchType}, Depth: ${metadata.researchDepth}, Generated: ${new Date(metadata.generatedAt).toLocaleDateString()}`
    : '';
  return `${index + 1}. 🔬 "${doc.document?.name || 'Previous Research'}" (${(doc.similarity * 100).toFixed(1)}% match)${researchInfo}
   ${doc.relatedTopic ? `🎯 Related to: ${doc.relatedTopic}` : ''}
   💡 Key Findings: "${doc.content.substring(0, 400)}${doc.content.length > 400 ? '...' : ''}"`;
}).join('\n\n')}

` : ''}${regularDocs.length > 0 ? `📄 DOCUMENT SOURCES (${regularDocs.length}):
${regularDocs.filter(doc => doc.similarity >= 0.4).map((doc, index) => 
...
${regularDocs.filter(doc => doc.similarity >= 0.25 && doc.similarity < 0.4).length > 0 ? `📑 SUPPORTING DOCUMENTS (${regularDocs.filter(doc => doc.similarity >= 0.25 && doc.similarity < 0.4).length}):
${regularDocs.filter(doc => doc.similarity >= 0.25 && doc.similarity < 0.4).map((doc, index) => 
...
${previousResearch.length > 0 ? '**CRITICAL**: You have access to previous research findings on related topics. Please:\n1. Reference and build upon previous research where relevant\n2. Identify gaps or contradictions with previous findings\n3. Note any evolution in understanding or new insights\n4. Cross-reference findings between current and previous research\n\n' : ''}Please carefully analyze and incorporate insights from the above ${previousResearch.length > 0 ? 'research findings and document' : 'document'} excerpts into your research. Use specific quotes, data points, and findings from these sources to support your analysis. Reference the document names when citing information.`;
          
          console.log(`✅ Enhanced context search found ${sortedDocs.length} relevant excerpts from ${totalResults} total matches`);
          console.log(`📊 Context breakdown: ${previousResearch.length} previous research, ${regularDocs.length} documents`);
        } else {
...
`;
  ...
  showDocumentManager() {
    // Track modal opening
          if (window.AppConfig && typeof window.AppConfig.trackModal === 'function') {
        window.AppConfig.trackModal('knowledge_base_manager', 'open', 'deepresearch');
      }
  ...
  async handleModalDocumentUpload(event) {
    ...
    // Track document upload
            if (window.AppConfig && typeof window.AppConfig.trackDocumentOperation === 'function') {
          window.AppConfig.trackDocumentOperation('upload_modal', 'multiple', true, `${files.length} files`);
        }
  ...
  async loadDocumentList() {
    ...
    try {
      ...
      documents.forEach(doc => {
        const docSize = this.formatFileSize(doc.size);
        const docDate = new Date(doc.createdAt).toLocaleDateString();
        
        // 🆕 Check if this is a generated research document
        const isGeneratedResearch = doc.metadata?.type === 'generated_research';
        const researchIcon = isGeneratedResearch ? '🔬' : '📄';
        const researchBadge = isGeneratedResearch ? `
          <div style="
        ...
        // 🆕 Enhanced metadata for research documents
        const researchMetadata = isGeneratedResearch ? `
          <div style="font-size: 11px; color: rgba(255,255,255,0.6); margin-top: 4px;">
            📋 ${doc.metadata.researchType} • ${doc.metadata.researchDepth} • ${doc.metadata.topicsCount} topics
            <br>🤖 ${doc.metadata.aiProvider} • ${new Date(doc.metadata.generatedAt).toLocaleDateString()}
          </div>
        ` : '';
        ...
        documentList.innerHTML += `
          <div class="document-item">
            <div class="document-info">
              <div class="document-name">${researchIcon} ${doc.name}</div>
              <div class="document-meta">
                Size: ${docSize} • Uploaded: ${docDate} • Type: ${doc.type}
                ${researchBadge}
                ${researchMetadata}
              </div>
            </div>
            <div class="document-actions">
              <button class="doc-action-btn" onclick="deepResearch.viewDocument('${doc.id}')">👁️ View</button>
  ...
  updateVectorStats() {
    ...
    try {
      ...
      // 🆕 Count generated research documents separately
      const documents = Array.from(this.vectorStore.collections.documents.values());
      const regularDocs = documents.filter(doc => doc.metadata?.type !== 'generated_research');
      const researchDocs = documents.filter(doc => doc.metadata?.type === 'generated_research');
      
      document.getElementById('statDocuments').textContent = regularDocs.length || 0;
      ...
      // 🆕 Update research outputs stat if element exists
      const statResearchElement = document.getElementById('statResearch');
      if (statResearchElement) {
        statResearchElement.textContent = researchDocs.length;
      } else if (researchDocs.length > 0) {
        // Add research stat dynamically if it doesn't exist and we have research docs
        const statsGrid = document.querySelector('.stats-grid');
        if (statsGrid) {
          const researchStatHtml = `
            <div class="stat-item">
              <div class="stat-value" id="statResearch">${researchDocs.length}</div>
              <div class="stat-label">Research Outputs</div>
            </div>
          `;
          statsGrid.innerHTML += researchStatHtml;
        }
      }
  ...
  /**
   * View document content with enhanced display for research outputs
   */
  async viewDocument(documentId) {
    try {
      ...
      // 🆕 Check if this is a generated research document
      const isGeneratedResearch = docData.metadata?.type === 'generated_research';
      ...
      const title = isGeneratedResearch ? 
        `🔬 Generated Research: ${docData.name}` : 
        `📄 Document: ${docData.name}`;
      
      const metadata = isGeneratedResearch ? `
        <div style="
          background: linear-gradient(45deg, #4facfe 0%, #00f2fe 100%);
          color: white;
          padding: 15px;
        ...
        ">
          ...
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; margin-top: 10px;">
            <div><strong>Research Type:</strong> ${docData.metadata.researchType}</div>
            <div><strong>Depth:</strong> ${docData.metadata.researchDepth}</div>
            <div><strong>Topics:</strong> ${docData.metadata.topicsCount}</div>
            <div><strong>AI Provider:</strong> ${docData.metadata.aiProvider}</div>
            <div><strong>Generated:</strong> ${new Date(docData.metadata.generatedAt).toLocaleDateString()}</div>
            <div><strong>Document Integration:</strong> ${docData.metadata.hasDocumentIntegration ? 'Yes' : 'No'}</div>
          </div>
        </div>
      ` : `
        <div style="
      ...
      modal.innerHTML = `
        <div class="document-modal-content" style="max-width: 1000px; max-height: 90vh;">
          <div class="document-modal-header">
            <h2>${title}</h2>
            <button class="close-modal" onclick="this.closest('.document-modal').remove()">✕</button>
          </div>
          ...
          <div style="display: flex; gap: 10px; margin-top: 20px;">
            <button class="btn secondary" onclick="this.closest('.document-modal').remove()">Close</button>
            ${isGeneratedResearch ? `
              <button class="btn primary" onclick="deepResearch.downloadResearchDocument('${documentId}')">
                📥 Download Research
              </button>
            ` : `
              <button class="btn primary" onclick="deepResearch.downloadDocument('${documentId}')">
                📥 Download
              </button>
            `}
          </div>
  ...
  /**
   * Download research document with proper formatting
   */
  async downloadResearchDocument(documentId) {
    try {
      ...
      console.error('Failed to download research document:', error);
      alert('Failed to download document: ' + error.message);
    }
  }
  
  /**
   * Download regular document
   */
  ...
  async quickSearch() {
    ...
    // Track quick search
          if (window.AppConfig && typeof window.AppConfig.trackDocumentOperation === 'function') {
        window.AppConfig.trackDocumentOperation('quick_search', 'search', true, query.substring(0, 50));
      }
  ...
  closeDocumentModal() {
    ...
    // Track modal closing
    if (window.AppConfig && typeof window.AppConfig.trackModal === 'function') {
      window.AppConfig.trackModal('knowledge_base_manager', 'close', 'deepresearch');
    }
