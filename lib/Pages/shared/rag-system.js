...
class SharedRAGSystem {
  ...
  isRAGQuery(message) {
    ...
    return ragKeywords.some(keyword => message.toLowerCase().includes(keyword));
  }
  ...
  async generateRAGResponse(query, aiAssistant, options = {}) {
    ...
    try {
      ...
      const prompt = `Answer this question based on the provided document context:
...
Instructions:
- Provide a comprehensive answer based ONLY on the document context
