...
class SharedAIConnectionManager {
  constructor() {
    ...
    // Throttling for restoration attempts
    this.lastRestoreAttempt = 0;
    this.restoreThrottleMs = 5000; // 5 seconds between restore attempts
    this.isRestoring = false;
    this.lastRefreshTime = 0;
    this.refreshThrottleMs = 2000; // 2 seconds between refresh attempts
  ...
  /**
   * Restore AI assistant from saved connection state
   */
  async restoreAIAssistant() {
    ...
    // Throttle restoration attempts
    if (now - this.lastRestoreAttempt < this.restoreThrottleMs) {
      console.log('⏸️ Restoration throttled - too recent attempt');
      return;
    }
    ...
    try {
      if (!this.currentProvider) {
        console.log('❌ No provider to restore');
        return;
      }
      ...
      // Set up status change callback first
      this.setupStatusChangeCallback();
      ...
      if (restored) {
        ...
        console.log('❌ Failed to restore AI connection');
        this.clearConnectionState();
      }
      
    } catch (error) {
      console.error('❌ Error during AI assistant restoration:', error);
  ...
  async refreshConnectionState() {
    ...
    if (now - this.lastRefreshTime < this.refreshThrottleMs) {
      console.log('⏸️ Refresh throttled - too recent');
      return;
    }
