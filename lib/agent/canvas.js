  ...
  async initialCodeGeneration(userPrompt) {
    ...
    try {
      ...
      try {
        ...
        this.log('📄 Raw API response content', 'info', { 
        ...
        this.log('❌ JSON parsing failed, attempting content extraction', 'error', { 
        ...
        if (codeMatch) {
          ...
          this.log('✅ Code extracted from malformed JSON', 'success', { codeLength: extractedCode.length });
          return extractedCode;
        } else {
          throw new Error(`JSON parsing failed: ${jsonError.message}. Raw content: ${rawContent.substring(0, 200)}...`);
        }
      }
...
3. NEVER call createCanvas() outside the draw function
4. Use t+=0.01 to 0.1 for smooth animation
5. Create VISUALLY RICH patterns using mathematical functions
6. Use multiple layers of animation for depth
7. AVOID: let/const declarations, complex transformations, nested push/pop
8. USE: Direct coordinate calculations, sin/cos patterns, color gradients
      ...
      try {
        ...
        testP5Instance = new p5((p) => {
          ...
          p.createCanvas = function(w, h) {
            // Use our test canvas - simplified approach
            p.width = w || 400;
            p.height = h || 400;
            p.canvas = testCanvas;
            p.drawingContext = testCanvas.getContext('2d');
            return testCanvas;
          };
...
class CanvaScan {
  ...
  async analyzeCode(code, originalPrompt) {
    ...
    // Critical p5.js function validation
    if (code.includes('createRenderer')) issues.push('CRITICAL: createRenderer() does not exist in p5.js - use createCanvas()');
    if (code.includes('renderer(')) issues.push('CRITICAL: renderer() does not exist in p5.js - use createCanvas()');
    
    // Check for createCanvas outside draw function (common AI mistake)
    if (code.includes('createCanvas(') && !code.includes('t||createCanvas(')) {
      issues.push('CRITICAL: createCanvas() must be inside draw function with conditional: t||createCanvas()');
    }
    ...
    if (!code.includes('createCanvas') && !code.includes('createRenderer')) issues.push('Missing canvas creation');
  ...
  async repairCode(code, issues, originalPrompt) {
    ...
    // Quick fix for critical p5.js function errors before AI repair
    let quickFixedCode = code;
    if (code.includes('createRenderer')) {
      quickFixedCode = quickFixedCode.replace(/createRenderer\s*\(/g, 'createCanvas(');
      this.core.log('🔍 CanvaScan: Applied quick fix - replaced createRenderer with createCanvas', 'info');
    }
    if (code.includes('renderer(')) {
      quickFixedCode = quickFixedCode.replace(/renderer\s*\(/g, 'createCanvas(');
      this.core.log('🔍 CanvaScan: Applied quick fix - replaced renderer with createCanvas', 'info');
    }
    
    // Fix createCanvas outside draw function (common AI mistake)
    if (quickFixedCode.includes('createCanvas(') && !quickFixedCode.includes('t||createCanvas(')) {
      this.core.log('🔍 CanvaScan: Detected createCanvas outside draw function, applying fix...', 'info', { originalCode: quickFixedCode.substring(0, 200) });
      
      // Pattern: t=0;createCanvas(w,h);draw=_=>{...
      const fixedCode = quickFixedCode.replace(
        /(t\s*=\s*0\s*;?\s*)createCanvas\s*\(\s*(\d+)\s*,\s*(\d+)\s*\)\s*;?\s*(draw\s*=\s*[^{]*\{)/g,
        '$1$4\n  t||createCanvas(w=$2,h=$3)\n  '
      );
      
      if (fixedCode !== quickFixedCode) {
        quickFixedCode = fixedCode;
        this.core.log('🔍 CanvaScan: Applied quick fix - moved createCanvas inside draw function', 'success');
      } else {
        // Try alternative pattern for cases like: t=0;createCanvas(800,600);draw=_=>{
        const altFixed = quickFixedCode.replace(
          /^(.*?)(createCanvas\s*\([^)]+\)\s*;?\s*)(draw\s*=)/,
          '$1$3'
        ).replace(
          /(draw\s*=\s*[^{]*\{\s*)/,
          '$1\n  t||createCanvas(w=800,h=600)\n  '
        ...
        if (altFixed !== quickFixedCode) {
          quickFixedCode = altFixed;
          this.core.log('🔍 CanvaScan: Applied alternative fix - removed external createCanvas and added conditional', 'success');
        } else {
          this.core.log('🔍 CanvaScan: Could not apply createCanvas fix automatically', 'warning');
        }
      }
    }
    
    const repairPrompt = this.buildRepairPrompt(quickFixedCode, issues, originalPrompt);
...
CRITICAL P5.JS FUNCTION CORRECTIONS:
- createRenderer() → createCanvas() (createRenderer does NOT exist in p5.js!)
- renderer() → createCanvas()
- canvas() → createCanvas()
- setup() → t||createCanvas() (use conditional setup)
- createCanvas(w,h) outside draw → t||createCanvas(w,h) inside draw function

CRITICAL STRUCTURE REQUIREMENTS:
- createCanvas() MUST be inside the draw function with conditional: t||createCanvas(w=800,h=600)
- NEVER call createCanvas() at global scope
- Pattern: t=0\ndraw=_=>{\n  t||createCanvas(w=800,h=600)\n  // rest of code\n}

REPAIR REQUIREMENTS:
1. Fix all detected issues
2. CRITICAL: Replace any createRenderer() calls with createCanvas()
3. Maintain the original creative intent
4. Use only compact syntax (t=0, draw=_=>{...})
5. Ensure code follows SketchPad-SLM patterns
6. Keep visual appeal and animation quality
7. Use correct p5.js functions only
