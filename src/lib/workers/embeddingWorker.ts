...
class EmbeddingWorker {
  ...
  async init() {
    try {
      console.log("🔧 Web Worker: Initializing BGE embedding pipeline...");
      console.log(
        "🚀 Using BAAI/bge-small-en-v1.5 for improved semantic understanding"
      );

      // Initialize the enhanced BGE embedding pipeline in the worker
      ...
      console.log(
        "✅ Web Worker: BGE embedding pipeline initialized successfully"
      );
      console.log(
        "📊 Expected performance improvement: +7% better semantic search quality"
      );
      ...
      console.error(
        "❌ Web Worker: Failed to initialize embedding pipeline:",
        error
      );
      self.postMessage({
        type: "error",
  ...
  async processDocument(documentData: {
    ...
    try {
      ...
      console.log(
        `📊 Web Worker: Created ${chunks.length} chunks, processing first ${maxChunks}`
      );

      if (chunks.length > 200) {
        console.warn(
          `⚠️ Web Worker: Large document with ${chunks.length} chunks, limiting to first 200 chunks`
        );
      }
      ...
      for (let i = 0; i < chunksToProcess.length; i += batchSize) {
        ...
        const batchPromises = batch.map(async (chunk, batchIndex) => {
          ...
          try {
            ...
            console.error(
              `❌ Web Worker: Failed to process chunk ${globalIndex}:`,
              chunkError
            );
            return null;
          }
      ...
      console.log(
        `✅ Web Worker: Document processing complete - ${processedChunks.length} chunks processed`
      );
    } catch (error) {
  ...
  private createWordBasedChunks(
    ...
    overlapWords: number = 0
  ): Array<{ content: string; wordCount: number; hasOverlap: boolean }> {
    const words = text
      .trim()
      .split(/\s+/)
    ...
    console.log(
      `📊 TS Worker: Chunking ${words.length} words into ${wordsPerChunk}-word chunks with ${overlapWords} word overlap`
    );
